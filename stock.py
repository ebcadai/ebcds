...
#     today = date.today()
#     pdraw = pdr.get_data_yahoo(symb, start="2010-01-01", end=today)
#     old = pdraw.reset_index()
#     for i in ['Open', 'High', 'Close', 'Low']:
#         old[i] = old[i].astype('float64')
#     fig = Figure()
#     canvas = FigureCanvas(fig)
#     ax = go.Figure(data=[go.Candlestick(x=old['Date'], open=old['Open'], high=old['High'], low=old['Low'], close=old['Close'])])
#     html_graph_c = mpld3.fig_to_html(ax)
#     return html_graph_c
